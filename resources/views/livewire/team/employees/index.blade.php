<div>
    <div class="flex items-center justify-between flex-wrap gap-2 mb-6">
        ...
        @if($user->isSuperAdmin() || $user->can('employees.create'))
            <div>
                @include('livewire._partials.button_create',[
                              'text' => __('Add new'), 'url'=> route('employees.create')])
            </div>
        @endif
    ...
    <div class="card overflow-hidden p-6">
        ...
        @if($count)
            <div class="flex mb-5 gap-2 justify-start items-center">
                ...
                @if(count($stores))
                    @include('livewire.team.employees._partials.filters')
                @endif
                @if(!empty($search) or !empty($fistoreFilter))
        ...
        @if(count($items))
            @include('livewire.team.employees._partials.data')
        @else
